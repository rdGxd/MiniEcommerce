...
export default function ProductCategoryDiagnostic() {
  ...
  const runDiagnostic = async () => {
    ...
    try {
      ...
      const productsWithoutCategories = products.filter(p => p.categories.length === 0).length;
      const relationshipsCount = products.reduce((sum, p) => sum + p.categories.length, 0);
  ...
  const generateSQLScript = () => {
    ...
    const productsWithoutCat = diagnostic.products.filter(p => p.categories.length === 0);
    ...
    productsWithoutCat.forEach(product => {
      ...
      if (categoryId) {
        associations.push(`  ('${product.id}', '${categoryId}') -- ${product.name}`);
      }
    });
  ...
  const getCategoryForProduct = (productName: string): string | null => {
    ...
    if (name.includes('mouse') || name.includes('teclado') || name.includes('monitor') ||
        name.includes('webcam') || name.includes('microfone')) {
      return '1f454fe3-d608-414c-a6d2-74e2c78f8087'; // Eletrônicos
    }

    if (name.includes('gamer') || name.includes('gaming') || name.includes('console') ||
        name.includes('controle') || name.includes('cadeira gamer')) {
      return 'a4e1b2c3-d4e5-f6g7-h8i9-j0k1l2m3n4o5'; // Gaming
    }

    if (name.includes('ssd') || name.includes('hd') || name.includes('processador') ||
        name.includes('placa') || name.includes('memória') || name.includes('fonte')) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ferramenta para diagnosticar e corrigir o problema de produtos sem categorias
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{diagnostic.totalProducts}</div>
                  <div className="text-sm text-gray-600">Total de Produtos</div>
                </div>
                <div className="text-center p-4 bg-red-50 rounded-lg">
                  <div className="text-2xl font-bold text-red-600">{diagnostic.productsWithoutCategories}</div>
                  <div className="text-sm text-gray-600">Sem Categorias</div>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{diagnostic.categoriesCount}</div>
                  <div className="text-sm text-gray-600">Categorias Disponíveis</div>
                </div>
                <div className="text-center p-4 bg-purple-50 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">{diagnostic.relationshipsCount}</div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <span className="bg-red-500 text-white px-2 py-1 rounded text-sm">{diagnostic.productsWithoutCategories}</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    .map(product => (
                      <div key={product.id} className="p-3 border rounded-lg">
                        ...
                        <div className="text-sm text-gray-600">
                          ID: {product.id} | Preço: R$ {product.price} | Estoque: {product.stock}
                        </div>
                      </div>
                    ))}
                  {diagnostic.productsWithoutCategories > 5 && (
                    <p className="text-sm text-gray-500">
                      ... e mais {diagnostic.productsWithoutCategories - 5} produtos
                    </p>
                  )}
                </div>
              </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {diagnostic.categories.map(category => (
                  <span key={category.id} className="bg-gray-100 px-3 py-2 rounded-lg text-sm border">
                    {category.name}
                  </span>
                ))}
              </div>
          ...
          {diagnostic.productsWithoutCategories > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Execute este script no seu banco de dados para corrigir os problemas
                </CardDescription>
              </CardHeader>
              <CardContent>
